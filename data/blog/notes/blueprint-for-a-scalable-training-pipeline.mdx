...
To avoid this trap, the **recommended approach** is to use `link_arguments` in `LightningCLI`, allowing `num_classes` or other parameters computed in the `DataModule` to be passed to the `LightningModule` during instantiation. However, since `prepare_data()` runs **after** `__init__()`, parameters like `num_classes` might not be available in time. As a workaround by defining `num_classes` as a `@property`, you ensure it is “lazily” computed when first accessed, guaranteeing it’s initialized properly before model construction – as discussed [here](https://github.com/Lightning-AI/pytorch-lightning/discussions/13403). Lightning tracks the execution of `prepare_data()`, ensuring it runs only once, even if accessed multiple times via a property.
...
```python:data.py {12-13, 34-35, 37-38, 44-45, 51-55, 58-63} showLineNumbers
import lightning as L
...
class ImageClassificationModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        self.all_data = load_data(self.data_dir)  # Assumed function to load dataset to memory
        self.num_classes = get_num_classes(self.all_data)  # Assumed function to calculate num_classes
        self.class_weights = get_class_weights(self.all_data)  # Assumed function to calculate class weights
...
You can see that here we use custom data loading and splitting logic inside the `setup()` method and instantiate `PyTorch Dataclass` with the splits and corresponding transforms to avoid the need for an extra wrapper class as discussed in the first section.
...
- To avoid using extra wrapper classes for dataset splitting and transformations, handle data loading outside the `PyTorch Dataset` class.
- PyTorch Lightning organizes training into `LightningModule` and `LightningDataModule`, making it scalable but introduces a challenge when sharing parameters between modules.
...
- Put data loading in the `prepare_data()` method and use a `@property` workaround to ensure `prepare_data()` is executed before `link_arguments` passes the parameter.
