...
images:
  [
    'static/images/ema/exponential_weights_2row_barplot.png',
    'static/images/machine-learning/algo.png',
  ]
draft: false
...
where again $w_k = \alpha(1-\alpha)^{t-k}$. **Note that the first term is differently weighted**; this is the weak point of the method. Before we address this issue, let’s plot the weight values in the function of the sample number for $\alpha=0.1$ and $\alpha = 0.2$.
...
where the weights sum up to $1$ by definition. Note that in this definition we sum the observations starting at the latest and going back in time: $x_t, \ x_{t-1},\dots, \ x_0$. It has a weighted sum in the numerator and the cumulative weights in the denominator. If we substitute $w_k =(1-\alpha)^k$ into the above equation we get the **adjusted EMA formula** which behaves better for small sample sizes:
...
&=
\alpha x_t + \left[(1 -\alpha)x_{t-1} + (1-\alpha)^2x_{t-2}+\dots\right]\alpha \\
...
where $u_t$ denotes the nominator and $v_t$ the denominator in Eq. $(2.2)$. A didactical Python implementation for both the traditional and the adjusted method would look like as follows:
...
$$
c^{\text{EMA}} = \frac{\sum_{k=0}^{\infty} k w_k}{\sum_{k=0}^{\infty} w_k} = \sum_{k=0}^{\infty} k w_k \quad (\text{since } \sum w_k = 1) \tag{2.2}
$$

where we exploited the fact that the sum of the weights of a weighted average should be one. We saw that the the adjusted formula Eq. $(2.2)$ and the traditional formula Eq. $(1.3)$ is asymptotically equivalent. For simplicity, work with the weight definition of the latter: $w_k = \alpha(1-\alpha)^k$ and substitute it to Eq. $(2.2)$:

$$
c^{\text{EMA}} = \sum_{k=0}^{\infty} k w_k =\sum_{k=0}^\infty k \alpha(1-\alpha)^k \tag{2.4}
$$
...
\sum_{k=0}^{\infty} k r^k = \frac{r}{(1 - r)^2} \quad \text{,where} \; r \in(0,1). \tag{2.5}
$$

Consequently, Eq. $(2.4)$ renders to:

$$
c^{\text{EMA}} = \alpha \left( \frac{1-\alpha}{\left[1- (1 - \alpha)\right]^2} \right)=\alpha\left(\frac{1-\alpha}{\alpha^2}\right) = \frac{1-\alpha}{\alpha}. \tag{2.6}
$$
...
w_k^{\text{SMA}} = \frac{1}{n} \quad \text{for } k = 0 \text{ to } n - 1. \tag{2.7}
$$

So the CoM for SMA becomes:

$$
c^{\text{SMA}} = \sum_{k=0}^{n-1}k w_k^{\text{SMA}} =  \frac{1}{n}\sum_{k=0}^{n-1}k =  \frac{1}{n} \left( \frac{n(n - 1)}{2} \right) = \frac{n - 1}{2} \tag{2.8}
$$
